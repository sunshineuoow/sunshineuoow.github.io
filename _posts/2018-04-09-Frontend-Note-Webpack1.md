...
webpack 本质上是一个静态模块打包器(module bundler)。<br>
当 webpack 处理应用程序时，它会递归地构建一个包含你应用中所需的每个模块的依赖图，而后将这些模块打包成一个或多个 bundle。

## webpack 的快速构建（wepack4 新增）

webpack 会默认以`index.js`为入口文件，输出文件在`dist/main.js`
